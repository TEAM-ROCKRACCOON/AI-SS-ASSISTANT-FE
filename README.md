...
> 사용자 맞춤형 청소 일정 추천 및 시각적 동기부여 서비스입니다. :contentReference[oaicite:0]{index=0}  

자취 초년생의 **생활 패턴·일정·날씨·피드백**을 종합해  
“**지금 현실적으로 할 수 있는 청소 루틴**”을 자동으로 만들어 주는 웹 앱 개발 프로젝트입니다.
...
- PuLP 기반 **정수선형계획법(ILP)**으로 제약조건(요일·시간대 선호, 주간 총 청소 시간, 중복 작업 제한)을 만족하는 초기 청소 스케줄 생성 :contentReference[oaicite:1]{index=1}  
- Tianshou(PPO) 기반 자체 **강화학습 에이전트**가 사용자의 실행 이력·만족도 피드백을 학습하여, 다음 주 루틴을 점점 더 개인화

### 2. 날씨 기반 청소 추천
- OpenWeatherMap API로 거주 지역의 **기온·강수량·미세먼지/습도**를 조회 :contentReference[oaicite:2]{index=2}  
- “맑은 날엔 빨래”, “미세먼지 심한 날엔 환기 금지” 등 규칙 기반 추천으로 **날씨에 맞는 청소 루틴** 자동 제안

### 3. 시각 피드백 (Before → After)
- 사용자가 업로드한 방 사진에서 Grounded-SAM으로 **정돈이 필요한 물체**를 검출 후 마스킹 :contentReference[oaicite:3]{index=3}  
